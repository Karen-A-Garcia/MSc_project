...
### ^^^^ Old script but I don't want to completely get rid of
...
# Load data
output_dir = "/home/karengarcia/downloads-karengarcia/ERA5/Monthly/"
shum_files = sorted(glob.glob(os.path.join(output_dir,"ERA5_monthly_q_1979_2019_128x64.nc")))
shum = xr.open_mfdataset(shum_files, chunks=chunks).isel(valid_time=slice(-12*20, None))
# print(shum)
# # # Grid spacing
d_lon = np.deg2rad(shum.lon.diff("lon").values[0])
print("In degrees", shum.lon.diff("lon").values[0])
d_lat = np.deg2rad(abs(shum.lat.diff("lat").values[0]))
print("In degrees", abs(shum.lat.diff("lat").values[0]))


# print(shum['pressure_level'].values)
shum = shum.sel(pressure_level=slice(100, 0))


# # # Pressure layer thicknesses
p_vals = shum["pressure_level"].values*100
edges = np.concatenate([[p_vals[0] - (p_vals[1] - p_vals[0]) / 2],
                        (p_vals[:-1] + p_vals[1:]) / 2,
                        [p_vals[-1] + (p_vals[-1] - p_vals[-2]) / 2]])

dP_all = xr.DataArray(np.abs(np.diff(edges)),
                      coords=[shum.pressure_level],
                      dims=["pressure_level"])

# Remove final level to match layer thicknesses
shum_above = shum["q"].isel(pressure_level=slice(0, -1))
dP_sliced = dP_all.isel(pressure_level=slice(0, -1))

# # Column water vapour (kg m^-2)
W_strat = (shum_above * dP_sliced).sum(dim="pressure_level") / g
# print(W_strat)
# # Grid-cell area
cell_area = (E_radius**2* np.cos(np.deg2rad(shum.lat))* d_lon*d_lat)

# # lat masks
tropics_mask = (shum.lat >= -30) & (shum.lat <= 30)
extratropics_mask = (shum.lat > 30) | (shum.lat < -30)
 

# # Integrated masses
mass_global = (W_strat * cell_area).sum(dim=["lat", "lon"]).compute()
mass_tropics = (W_strat.where(tropics_mask) * cell_area).sum(dim=["lat", "lon"]).compute()
mass_extratropics = (W_strat.where(extratropics_mask) * cell_area).sum(dim=["lat", "lon"]).compute()

plt.figure(figsize=(16, 6))
# plt.plot(mass_global.valid_time,mass_global / 1e9,
#         color="black",
#         linewidth=2.5,
#         label="Global")

plt.plot(mass_tropics.valid_time,mass_tropics / 1e9,
        color="red",
        linewidth=2,
        label="30°S–30°N")

plt.plot(mass_extratropics.valid_time, mass_extratropics / 1e9,
        color="blue",
        linewidth=2,
        label="Extratropics")
...
plt.xlabel("Time")
plt.ylabel("Mass (Tg)")
plt.title("ERA5 Integrated Stratospheric Water Vapour Above 100 hPa")
plt.legend()
plt.tight_layout()

final_plot_path = ("/home/karengarcia/MSc_project/Figures/Water_budget/Control_volume/ERA5_20_year_Tropics_Extratropics_Above100hPa_124x64.png")
